...
public class ProductController : BaseController
{
    ...
    [Authorize]
    [HttpGet("")]
    public async Task<IActionResult> GetAllProductsAsync(string filterBy, string FilterFor, string Sort)
    {
        try
        {
            var result = await _mediator.Send(new GetProductRequest(filterBy, FilterFor, Sort));
            return Ok(result);
        }
        catch (Exception ex)
        {
